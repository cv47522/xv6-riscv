...
| ----------- | ------------- |
| **Program `sleep` in `user/sleep.c`** | The guest command is built as `user/_sleep` and packed into `fs.img`. |
...
```mermaid
flowchart TD
    A["user/sleep.c<br/>forms the request"] --> B["user/user.h<br/>declares int pause(int)"]
    B --> C["user/usys.S and kernel/syscall.c<br/>route syscall 13"]
    C --> D["kernel/sysproc.c<br/>sys_pause waits"]
    D --> E["normal return<br/>or killed process exits in usertrap"]
...
_Pink forms the request, blue routes it, yellow performs the wait, and cyan is the outcome._
...
| `kernel/syscall.c` | Dispatches call number 13 and transfers the return value through the trapframe. | No knowledge of the argument's meaning. |
...
| Code | Meaning |
| ---- | ------- |
| **`argint(0, &n)`** | Fetches argument register `a0` from the saved trapframe. Every syscall handler has the dispatcher-compatible form `uint64 f(void)`. |
...
| **`tickslock` and `ticks0`** | Protect the global counter and snapshot the starting tick. |
...
```mermaid
flowchart TD
    A["fetch n and clamp negatives to zero"] --> B["lock ticks and snapshot ticks0"]
    B --> C{"enough ticks elapsed?"}
    C -- yes --> G["unlock and return 0"]
    C -- no --> D{"process killed?"}
    D -- yes --> H["unlock and return -1<br/>usertrap exits process"]
    D -- no --> E["register &ticks<br/>unlock and sleep"]
    E --> F["wake and relock ticks"]
    F --> C
    ...
    classDef processing fill:#fffff0,stroke:#cc6
    classDef commit fill:#f0ffff,stroke:#6cc
    classDef stall fill:#ffd9d9,stroke:#c44
    class A request
    class B,C,D routing
    class F processing
    class E stall
